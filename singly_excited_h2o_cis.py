...
P = mf.make_rdm1()
print('rdm')
print_matrix_as_integers(P)
print('rdm from mo')
print_matrix_as_integers(C @ C.T)
...
D_C = 2 * Cvir @ Cvir.conj().T
D_B = 2 * Cenv @ Cenv.conj().T
D_A = 2 * Cact @ Cact.conj().T
...
emb_mf.verbose = 4
emb_mf.get_hcore =  lambda *args: H_core
...
J_emb, K_emb = emb_mf.get_jk()
V_emb = J_emb - 0.5 *K_emb


E_env = np.einsum('ij,ji->', D_B, H_core) + 0.5 * np.einsum('ij,ji->', D_B, Venv)
E_act = np.einsum('ij,ji->', D_A, H_core) + 0.5 * np.einsum('ij,ji->', D_A, Vact)
E_cross = 0.5 * np.einsum('ij,ji->', D_B, Vact) + 0.5 * np.einsum('ij,ji->', D_A, Venv)

E_emb = np.einsum('ij,ji->', P_emb, mf.get_hcore) + 0.5 * np.einsum('ij,ji->', P_emb, V_emb)
correction = np.einsum('ij,ji->', V_emb, (P_emb-D_A))
post = mu * np.einsum('ij,ji->', self.P_emb, P_B )

embed_SCF = E_emb + E_env + E_cross + correction + emb_mf.Vnn + post

print('sanitycheck', embed_SCF - E_init)

# CIS
# compute singlets
mf_eff = tdscf.TDA(emb_mf)
mf_eff.singlet = True
mf_eff = mf_eff.run(n_states =1)
mf_eff.analyze()
